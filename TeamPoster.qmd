...
# Select only those from the year 2023
summarized_cleaned_data_2023 <- summarized_cleaned_data_UPPER %>%
  filter(Time == 2023)
...
merged_data <- singapore_map_clean %>%
  left_join(summarized_cleaned_data_2023, by = c("Name" = "PA"))
...
leaflet(merged_data) %>%
  addPolygons(
    ...
    label = ~paste("Planning Area: ", Name, " Total Population (2023): ", TotalPop)
  ) %>%
  addLegend(
