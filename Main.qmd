...
how do we see the content inside the kml file

```{r}
# Display the KML data
singapore_map
```
First import the cleaned_data.csv

```{r}
library(readr)
# Load the cleaned data
aggregated_df <- read_csv("cleaned_data.csv")
...
Next we want to use the names in the PA column in the cleaned_data to match with the kml file so that the TotalPop in the cleaned_data can be plotted on the map. We will then plot the map with the TotalPop data.
...
# Plot the map with the TotalPop data
ggplot(data = merged_data) +
  geom_sf(aes(fill = TotalPop)) +
  theme_minimal() +
  labs(title = "Population Density by Planning Area in Singapore",
