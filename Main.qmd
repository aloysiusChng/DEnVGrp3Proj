...
```{r}
singapore_map_viewer <- st_read(kml_file_path)
singapore_map_viewer
```
...
plot_ly(data = singapore_map_clean) %>%
  add_sf(
    aes(fill = ~st_area(geometry), 
...
```{r}
#Define the path to your KML file (ensure this path is correct)
kml_file_path <- "ElectoralBoundary2020KML.kml"
...
# Merge the cleaned data with the KML data based on the Planning Area column
merged_data <- merge(singapore_map, aggregated_df, by.x = "Name", by.y = "PA", all.x = TRUE)
...
singapore_geojson <- geojson_json(singapore_map_clean)
```
...
# Create the choroplethmapbox plot
plot_ly() %>%
  add_trace(
    type = "choroplethmapbox",
    geojson = singapore_geojson,
    locations = ~no_features,
    z = ~st_area(geometry),
    text = ~paste("Planning Area: ", no_features),
    colorscale = "Viridis",
...
# Create sample data for different years (assuming 'Year' is a column in your data)
# You should modify this to reflect your actual data structure
singapore_map_clean$Year <- sample(2000:2020, nrow(singapore_map_clean), replace = TRUE)

# Split the data by year
data_by_year <- split(singapore_map_clean, singapore_map_clean$Year)

# Create a leaflet map
leaflet() %>%
  addTiles() %>%
  addMapPane("polygons", zIndex = 410) %>%
  lapply(names(data_by_year), function(year) {
    addPolygons(
      ...
      opacity = 1.0,
      fillOpacity = 0.5,
      fillColor = ~colorQuantile("YlGnBu", NULL)(st_area(data_by_year[[year]])),
      highlightOptions = highlightOptions(
        color = "white",
        weight = 2,
        bringToFront = TRUE
  ...
  addLayersControl(
    overlayGroups = names(data_by_year),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  addTimeSlider(
    timeIntervals = names(data_by_year),
    position = "bottomleft",
    label = "Year",
    min = min(as.integer(names(data_by_year))),
    max = max(as.integer(names(data_by_year))),
    value = c(min(as.integer(names(data_by_year))), max(as.integer(names(data_by_year)))),
    step = 1,
    animate = TRUE,
